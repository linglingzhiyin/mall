...
import com.mall.common.bean.PicUploadResult;
...
public class PicUploadController {
	...
	//@Value是获取当前容器中读取到的配置文件的值，无法获取其他容器加载的值，即使是父容器application
	//父子容器：父容器为spring容器(通常为application，通过扫包确定容器范围)
	//     子容器为SpringMVC容器 (通过扫描Controller包确定mvc容器范围就是Controller)
	//     Controller能注入Service依赖，而service不能注入Controller依赖
	// 子容器能访问父容器的资源，而父容器无法访问紫容器的资源
	...
	public String upload(@RequestParam("uploadFile") MultipartFile uploadFile , HttpServletResponse response) throws Exception {
		...
		// 生成图片的绝对引用地址
		String picUrl = StringUtils.replace(StringUtils.substringAfter(filePath, propertiesService.getREPOSITORY_PATH()),
				"\\", "/");
		fileUploadResult.setUrl(propertiesService.getIMAGE_BASE_URL() + picUrl);
	...
	private String getFilePath(String sourceFileName) {
		String baseFolder = propertiesService.getREPOSITORY_PATH() + File.separator + "images";
