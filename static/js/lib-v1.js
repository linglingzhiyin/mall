    ...
    a.fn.Jlazyload = function (b, c) {
        if (this.length) {
            var d, e, f = a.extend({
                ...
                placeholderImage: "http://static.mall.com/images/blank.gif",
...
var category = {
    ...
    URL_Serv: "http://manage.mall.com/rest/api/item/cat?callback=category.getDataService",
    ...
    FN_GetLink: function (a, b) {
        ...
        return "" == c ? d : (/^http[s]?:\/\/([\w-]+\.)+[\w-]+([\w-.\/?%&=]*)?$/.test(c) || (c = c.replace(/-000$/, ""), c = c.match(/^\d*-\d*$/) ? "http://channel.jd.com/" + c + ".html" : "http://www.mall.com" + c), '<a href="' + c + '">' + d + "</a>")
    },
